<template>
  <div class="process-page">
    <!-- 상단내비게이션바 -->
    <nav class="navbar">
      <div class="nav-brand" @click="goHome">PolicySim</div>
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">그래프데이터로드 중...</p>
          </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-text">대기온톨로지 생성</p>
            <p class="waiting-hint">생성완료후자동시작구축그래프</p>
          </div>
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-hint">데이터를 표시하고 있습니다...</p>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">인터페이스 설명</div>
                <div class="detail-content">
                  문서를 업로드한 후, LLM이 문서 내용을 분석하여 자동으로 적합한 여론 시뮬레이션의 온톨로지 구조(개체 유형 + 관계 유형)를 생성합니다.
                </div>
              </div>
              
              <!-- 온톨로지 생성진행률 -->
              <div class="detail-section" v-if="ontologyProgress && currentPhase === 0">
                <div class="detail-label">생성진행률</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">생성의개체 유형 ({{ projectData.ontology.entity_types?.length || 0 }})</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">생성의관계유형 ({{ projectData.ontology.relation_types?.length || 0 }})</div>
                <div class="relation-list">
                  ...
                  <div v-if="(projectData.ontology.relation_types?.length || 0) > 5" class="relation-more">
                    +{{ projectData.ontology.relation_types.length - 5 }} 더 많은 관계...
                  </div>
                </div>
              </div>
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">대기온톨로지 생성...</div>
              </div>
            </div>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">인터페이스 설명</div>
                <div class="detail-content">
                  기반으로 생성된 온톨로지, 문서를 분할한 후 Zep API를 호출하여 지식 그래프를 구축하고, 개체와 관계를 추출합니다.
                </div>
              </div>
              
              <!-- 대기온톨로지완료 -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">대기온톨로지 생성완료...</div>
              </div>
              
              <!-- 구축진행률 -->
              <div class="detail-section" v-if="buildProgress && currentPhase >= 1">
                <div class="detail-label">구축진행률</div>
              ...
              <div class="detail-section" v-if="graphData">
                <div class="detail-label">구축결과</div>
                <div class="build-result">
                  <div class="result-item">
                    <span class="result-value">{{ graphData.node_count }}</span>
                    <span class="result-label">개체 노드</span>
                  </div>
                  <div class="result-item">
                  ...
                  <div class="result-item">
                    ...
                    <span class="result-label">개체 유형</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                ...
                <div class="phase-api">다음 단계로 진입 준비 중</div>
              </div>
        ...
        <div class="project-panel">
          ...
          <div class="project-details" v-if="projectData">
            ...
            <div class="project-item" v-if="projectData.graph_id">
              <span class="item-label">그래프ID</span>
...
const ontologyProgress = ref(null) // 온톨로지 생성진행률
const currentPhase = ref(-1) // -1: 업로드중, 0: 온톨로지 생성 중, 1: 그래프 구축, 2: 완료
...
// 처리새로 생성프로젝트 - 호출 ontology/generate API
const handleNewProject = async () => {
  const pending = getPendingUpload()
  
  if (!pending.isPending || pending.files.length === 0) {
    error.value = '업로드 대기 중인 파일이 없으면, 홈으로 돌아가서 다시 작업하세요.'
    loading.value = false
    return
  }
  ...
  try {
    ...
    ontologyProgress.value = { message: '진행 중: 파일 업로드 및 문서 분석 중...' }
    ...
    if (response.success) {
      ...
      error.value = response.error || '온톨로지 생성실패'
    }
  } catch (err) {
    console.error('Handle new project error:', err)
    error.value = '프로젝트초기화실패: ' + (err.message || '알 수 없음오류')
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || '로드프로젝트실패'
    }
  } catch (err) {
    console.error('Load project error:', err)
    error.value = '로드프로젝트실패: ' + (err.message || '알 수 없음오류')
  } finally {
    loading.value = false
  }
}
...
const updatePhaseByStatus = (status) => {
  switch (status) {
    ...
    case 'graph_completed':
      ...
      break
    case 'failed':
      error.value = projectData.value?.error || '처리실패'
      break
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      buildProgress.value.message = '그래프 구축작업이 시작되었습니다...'
      
      // 저장 task_id 용 라운드 요청
      const taskId = response.data.task_id
      
      // 시작 그래프 데이터 라운드 요청 (작업 상태와 독립적으로)
      startGraphPolling()
      
      // 시작 작업 상태 라운드 요청
      startPollingTask(taskId)
    } else {
      error.value = response.error || '시작그래프 구축실패'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = '시작그래프 구축실패: ' + (err.message || '알 수 없음오류')
    buildProgress.value = null
  }
}
...
const fetchGraphData = async () => {
  try {
    // 먼저 가져오기 프로젝트 정보하여 가져오기 graph_id  
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('✅ 그래프 구축 완료, 진행 중: 로드 전체 데이터...')  
        ...
        buildProgress.value = {
          progress: 100,
          message: '구축 완료, 진행 중: 로드그래프...'
        }
        
        // 다시 로드 프로젝트 데이터 가져오기 graph_id  
        const projectResponse = await getProject(currentProjectId.value)
        if (projectResponse.success) {
          ...
          if (projectResponse.data.graph_id) {
            console.log('📊 로드 전체 그래프:', projectResponse.data.graph_id)  
        ...
        stopPolling()
        stopGraphPolling()
        error.value = '그래프 구축실패: ' + (task.error || '알 수 없음오류')
        buildProgress.value = null
      }
    }
  } catch (err) {
...
const renderGraph = () => {
  ...
  simulation.on('tick', () => {
    // 업데이트 소 엣지의 위치 (포함 보기 선과 투명 클릭 영역)  
