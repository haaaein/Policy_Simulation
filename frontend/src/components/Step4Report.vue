            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text">진행 중: 생성{{ section.title }}...</span>
                </div>
              </div>
            </div>
          </div>
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (내용 생성 완료, 챕터 전체는 미완료) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // 대기 DOM 업데이트 후, 스크롤 위치를 조정하여 버튼에서 동일한 위치 유지
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // 전구 아이콘 - 통찰력을 나타냄
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // 지구 아이콘 - 전경 검색을 나타냄
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // 사용자 아이콘 - 대화를 나타냄
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // 번개 아이콘 - 빠름을 나타냄
  },
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // 차트 아이콘 - 통계를 나타냄
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // 데이터베이스 아이콘 - 개체를 나타냄
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    // 추출 분석 문제
    const queryMatch = text.match(/분석 문제:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // 추출 예측 장면
    const reqMatch = text.match(/예측 장면:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // 추출 통계 데이터 - "관련 예측 사실: X개" 형식에 맞춤
    const factMatch = text.match(/관련 예측 사실:\s*(\d+)/)
    const entityMatch = text.match(/관련 개체:\s*(\d+)/)
    const relMatch = text.match(/관계 체인:\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])
    
    // 추출 하위 질문 - 전체 추출, 수량 제한 없음
    const subQSection = text.match(/### 분석의하위 질문\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
      const lines = subQSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.subQueries = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }
    
    // 추출 주요 사실 - 전체 추출, 수량 제한 없음
    const factsSection = text.match(/### 【주요 사실】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (factsSection) {
    ...
    // 추출 핵심 개체 - 전체 추출, 요약 및 관련 사실 수 포함
    const entitySection = text.match(/### 【핵심개체】\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // 추출 역사/만료된 사실 - 전체 추출, 수량 제한 없음
    const histSection = text.match(/### 【역사\/만료된 사실】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
    ...
    // 추출 관련 개체 - 전체 추출, 수량 제한 없음
    const entitySection = text.match(/### 【관련 개체】\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    // 추출 인터뷰 메인 주제
    const topicMatch = text.match(/\*\*인터뷰 메인 주제:\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    // 추출 인터뷰 인원 수 (예: "5 / 9명 시뮬레이션 에이전트")
    const countMatch = text.match(/\*\*인터뷰인수:\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    // 추출인터뷰상선택이유
    const reasonMatch = text.match(/### 인터뷰상선택이유\n([\s\S]*?)(?=\n---\n|\n### 인터뷰실록)/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        } else if (currentName && line.trim() && !line.match(/^미선택|^종합|^최종선택/)) {
          // 이유의연속행（제외결말총결단락）
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // 추출답변 - 분Twitter와Reddit
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*중요인용|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // 분리Twitter와Reddit답변
        const twitterMatch = answerText.match(/【Twitter플랫폼답변】\n?([\s\S]*?)(?=【Reddit플랫폼답변】|$)/)
        const redditMatch = answerText.match(/【Reddit플랫폼답변】\n?([\s\S]*?)$/)
        ...
        // 플랫폼회귀로직（호환구형식：단일플랫폼표시의경우）
        if (!twitterMatch && redditMatch) {
          // 단지 Reddit 답변, 만약비점유텍스트일때복사로기본값표시
          if (interview.redditAnswer && interview.redditAnswer !== '（해당플랫폼미응답）') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '（해당플랫폼미응답）') {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch && !redditMatch) {
          // 없으면분플랫폼표시（극구형식）, 전체적으로답변
          interview.twitterAnswer = answerText
        }
      }
      
      // 추출중요인용（호환다양한인용형식）
      const quotesMatch = block.match(/\*\*중요인용:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // 우선매칭 > "text" 형식
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // 회귀：매칭 > "text" 또는 > \u201Ctext\u201D（중문인용부호）
        if (!quoteMatches) {
    ...
    // 추출 인터뷰 요약
    const summaryMatch = text.match(/### 인터뷰 요약와 핵심 관점\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    // 추출검색조회
    const queryMatch = text.match(/검색조회:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // 추출결과수량
    const countMatch = text.match(/찾\s*(\d+)\s*개/)
    if (countMatch) result.count = parseInt(countMatch[1])
    
    // 추출 관련 사실 - 완전 추출, 수량 제한 없음
    ...
    // 시도 추출 경계 정보（만약）
    const edgesSection = text.match(/### 관련 경계:\n([\s\S]*?)(?=\n###|$)/)
    if (edgesSection) {
    ...
    // 시도추출노드정보（만약）
    const nodesSection = text.match(/### 관련노드:\n([\s\S]*?)(?=\n###|$)/)
    if (nodesSection) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-header' }, [
        ...
        props.result.simulationRequirement && h('div', { class: 'header-scenario' }, [
          h('span', { class: 'scenario-label' }, '예측 장면: '),
      ...
      h('div', { class: 'insight-tabs' }, [
        ...
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `핵심개체 (${props.result.entities.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '시계열 기억 중 소관 연관의 최신 핵심 사실'),
          ...
          }, expandedFacts.value ? `접기 ▲` : `모두 펼치기 ${props.result.facts.length} 개 ▼`)
        ]),
        
        // Entities Tab
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '핵심개체'),
          ...
          }, expandedEntities.value ? `접기 ▲` : `모두 펼치기 ${props.result.entities.length} 개 ▼`)
        ]),
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          ...
          }, expandedRelations.value ? `접기 ▲` : `모두 펼치기 ${props.result.relations.length} 개 ▼`)
        ]),
        
        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '표류 조회 생성 분석 하위 질문'),
        ...
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, '핵심 개체가 없습니다'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, '관계 체인이 없습니다')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `현재 효능 기억 (${props.result.activeFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'historical' }],
          onClick: () => { activeTab.value = 'historical' }
        }, [
          h('span', { class: 'tab-label' }, `역사 기억 (${props.result.historicalFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, `관련 개체 (${props.result.entities.length})`)
        ])
      ]),
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '현재 효능 기억'),
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, '관련 개체가 없습니다'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '（해당 플랫폼응답 없음）' || t === '(해당 플랫폼응답 없음)' || t === '[응답 없음]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 만약 없으면 번호 찾기 또는 하나만 찾기, 전체로 돌아가기
      if (matches.length <= 1) {
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // 분할 실패: 첫 번째 문제로 전체 답변 반환, 나머지 빈 값 반환
      return qIdx === 0 ? answer : ''
    }
    
    // 특정 문제의 듀얼 플랫폼 답변 여부 확인（자리 표시 텍스트 필터）
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // 두 플랫폼 모두 실제 답변(플레이스홀더가 아닌 텍스트）이고 내용이 다름
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // 듀얼 플랫폼 전환 버튼（실제 듀얼 플랫폼 답변 시 표시）
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, '찾을 수 없음관련결과'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? `접기 ▲` : `모두 펼치기 ${props.result.facts.length} 개 ▼`)
...
const activeStep = computed(() => {
  ...
  // 만약없으면 active, 돌아가기 마지막 한 개 done 의 단계  
  const doneSteps = steps.filter(s => s.status === 'done')
  if (doneSteps.length > 0) return doneSteps[doneSteps.length - 1]
  
  // 그렇지 않으면 돌아가기 첫 번째 단계  
  return steps[0] || { noLabel: '--', title: '대기시작', status: 'todo', meta: '' }
})
...
const renderMarkdown = (content) => {
  ...
  // 시작의 2차 제목(## xxx)을 제거, 장 제목 이미에서 외부 표시  
  ...
  // 처리 줄 바꿈 - 빈 줄은 단락 구분으로, 단일 줄 바꿈은 <br>로 변환  
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // 정리 블록 수준 요소 전후의 <br> 태그  
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // 정리 <p><br> 블록 수준 요소의 경우(여분의 빈 줄 발생)  
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // 정리 연속의 <br> 태그  
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // 정리 블록 수준 요소 후 바로 다음 단락 시작 태그 전의 <br>  
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // 비연속 순 목록의 번호 수정: 단일 항목 <ol> 단락 내용이 분리될 때, 번호 증가 유지  
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // 보장 청소 loading 상태  
            emit('update-status', 'completed')
            stopPolling()
            // 스크롤 로직 통합에서 루프 종료 후 nextTick 중 처리  
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // 만약 작업 완료, 스크롤 상단; 그렇지 않으면 스크롤 하단 최신 로그 따라가기  
            if (isComplete.value) {
...
// 추출 최종 답변 내용 - 부터 LLM 응답 중 추출 장 내용  
const extractFinalContent = (response) => {
  if (!response) return null
  
  // 시도 추출 <final_answer> 태그 내의 내용  
  ...
  // 시도 찾기 Final Answer: 뒤의 내용(다양한 형식 지원)  
  // 형식1: Final Answer:\n\n내용
  // 형식2: Final Answer: 내용
...
/* 콘솔 로그 - 와 Step3Simulation.vue 유지 일치 */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
