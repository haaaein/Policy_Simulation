          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- 통용 회귀: 알 수 없는 유형 또는 content지만 위의 처리되지 않음 -->
...
const runStatus = ref({})
const allActions = ref([]) // 모든 액션 (증가 누적)
const actionIds = ref(new Set()) // 사용 중의 액션 ID 집합
const scrollContainer = ref(null)

// Computed
// 시간 순서에 따라 액션 표시 (최신에서 가장 아래, 즉 바닥)
...
// 형식화 시뮬레이션 경과 시간 (라운드 수와 각 라운드 분수에 따라 계산)
...
const resetAllState = () => {
  ...
  stopPolling()  // 중지 전에 저장된 라운드 조회
}

// 시작시뮬레이션
const doStartSimulation = async () => {
  if (!props.simulationId) {
    addLog('오류: simulationId가 누락됨')
    return
  }
  
  // 먼저 초기화된 상태, 이전 시뮬레이션의 영향을 받지 않도록 보장
  resetAllState()
  
  isStarting.value = true
  startError.value = null
  addLog('시작 중듀얼플랫폼병렬시뮬레이션...')
  ...
  try {
    const params = {
      ...
      platform: 'parallel',
      force: true,  // 강제로 재시작
      enable_graph_memory_update: true  // 동적으로 그래프 업데이트 활성화
    }
    ...
    addLog('이미 동적으로 그래프 업데이트 모드가 활성화됨')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ 이미 이전 시뮬레이션 로그를 정리했으며, 재시뮬레이션 시작')
      }
      ...
      addLog(`✗ 시작 실패: ${res.error || '알 수 없음오류'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(`✗ 시작예외: ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(`중지 실패: ${res.error || '알 수 없음오류'}`)
    }
  } catch (err) {
    addLog(`중지예외: ${err.message}`)
  } finally {
    isStopping.value = false
  }
}
...
// 각 플랫폼의 마지막 라운드 수를 추적하고 변화 감지 및 로그 출력
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 시뮬레이션이 완료되었는지 감지 (runner_status 또는 플랫폼 완료 상태로 판단)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // 추가 검사: 만약 백엔드가 아직 runner_status를 업데이트하지 않았지만 플랫폼이 이미 완료 보고
      // twitter_completed와 reddit_completed 상태로 판단
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ 감지된 플랫폼 시뮬레이션이 이미 종료됨')
        }
        addLog('✓ 시뮬레이션완료')
        phase.value = 2
        stopPolling()
    ...
    console.warn('가져오기실행상태실패:', err)
  }
}

// 활성화된 플랫폼이 완료되었는지 검사
const checkPlatformsCompleted = (data) => {
  // 만약 어떤 플랫폼 데이터도 없으면, false 반환
  ...
  // 만약 최소한 하나의 플랫폼이 완료되었다면, 활성화된 플랫폼이 모두 완료되었는지 검사
  // actions_count로 플랫폼이 활성화되었는지 판단 (만약 count > 0 또는 running이 true인 경우)
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // 만약 어떤 플랫폼도 활성화되지 않았다면, false 반환
  if (!twitterEnabled && !redditEnabled) return false
  
  // 활성화된 플랫폼이 모두 완료되었는지 검사
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 자동 스크롤, 사용자 자유 조회 시간 축  
      // 새 액션에서 바닥 추가  
    }
  } catch (err) {
    console.warn('가져오기 상세 상태 실패:', err)  
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('보고서 생성 요청이 이미 전송되었습니다, 잠시 기다려 주십시오...')
    return
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ 보고서 생성작업이 시작되었습니다: ${reportId}`)
      
      // 이동보고서페이지
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ 시작보고서 생성실패: ${res.error || '알 수 없음오류'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ 시작보고서 생성예외: ${err.message}`)
    isGeneratingReport.value = false
  }
}
