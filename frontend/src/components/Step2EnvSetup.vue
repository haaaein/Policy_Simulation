<template>
  <div class="env-setup-panel">
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">시뮬레이션인스턴스초기화</span>
          </div>
        ...
        <div class="card-content">
          ...
          <div v-if="simulationId" class="info-card">
            ...
            <div class="info-row">
              ...
              <span class="info-value mono">{{ taskId || '비동기작업완료' }}</span>
            </div>
          </div>
        </div>
      </div>

      <!-- Step 02: 생성 Agent 페르소나 -->
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        <div class="card-header">
          <div class="step-info">
            <span class="step-num">02</span>
            <span class="step-title">생성 Agent 페르소나</span>
          </div>
          <div class="step-status">
        ...
        <div class="card-content">
          ...
          <p class="description">
            컨텍스트를 결합하여, 자동 호출 도구부터 지식 그래프 개체와 관계를 정리하고, 초기화 시뮬레이션 개체를 만들며, 이를 기반으로 현실 시드에 그들에게 독특한 행로와 기억을 부여합니다.
          </p>
          ...
          <div v-if="profiles.length > 0" class="profiles-preview">
            <div class="preview-header">
              <span class="preview-title">이미생성의 Agent 페르소나</span>
            </div>
            <div class="profiles-list">
              ...
              >
                ...
                <div class="profile-meta">
                  <span class="profile-profession">{{ profile.profession || '알 수 없음직업' }}</span>
                </div>
      ...
      <!-- Step 03: 생성듀얼플랫폼시뮬레이션설정 -->
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        <div class="card-header">
          <div class="step-info">
            <span class="step-num">03</span>
            <span class="step-title">생성듀얼플랫폼시뮬레이션설정</span>
          </div>
          <div class="step-status">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM에 따라 시뮬레이션 요구사항과 현실 시드, 지능형 설정 월드 시간 흐름 속도, 추천 알고리즘, 각 개체의 활성 시간대, 발언 빈도, 이벤트 트리거 등의 파라미터를 설정합니다.
          </p>
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-grid">
                ...
                <div class="config-item">
                  <span class="config-item-label">각 라운드 시간</span>
                ...
                <div class="config-item">
                  <span class="config-item-label">매 시간 활성</span>
              ...
              <div class="platforms-grid">
                <div v-if="simulationConfig.twitter_config" class="platform-card">
                  <div class="platform-card-header">
                    <span class="platform-name">플랫폼 1：광장 / 정보 흐름</span>
                  </div>
                  <div class="platform-params">
                    ...
                    <div class="param-row">
                      <span class="param-label">열기 가중치</span>
                    ...
                    <div class="param-row">
                      <span class="param-label">바이러스 임계값</span>
                ...
                <div v-if="simulationConfig.reddit_config" class="platform-card">
                  <div class="platform-card-header">
                    <span class="platform-name">플랫폼 2：주제 / 사회 영역</span>
                  </div>
                  <div class="platform-params">
                    <div class="param-row">
                      <span class="param-label">시효 가중치</span>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            기반으로 서사 방향, 자동 생성 초기 활성화 이벤트와 핫 주제를 생성하여 시뮬레이션 월드의 초기 상태를 유도합니다.
          </p>
          ...
          <div v-if="simulationConfig?.event_config" class="orchestration-content">
            ...
            <div class="topics-section">
              <span class="box-label">초기 핫 주제</span>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <p class="description">시뮬레이션환경이미준비 완료，하여시작실행시뮬레이션</p>
          
          <!-- 시뮬레이션 라운드 수 설정 - 설정 생성 완료 후 라운드 수 계산 후에만 표시 -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">시뮬레이션 라운드 수 설정</span>
                <span class="section-desc">PolicySim 자동 계획 추론 현실 <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> 시간, 매 라운드는 현실 <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> 분의 시간 흐름을 나타냅니다</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>만약 Agent 규모가 100이라면: 예상 소요 시간 약 {{ Math.round(customMaxRounds * 0.6) }} 분</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        만약 Agent 규모가 100이라면: 예상 소요 시간 {{ Math.round(autoGeneratedRounds * 0.6) }} 분
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">첫 실행 시, 강력히 ‘사용자 정의 모드’로 전환하여 시뮬레이션 라운드 수를 줄이고, 빠르게 효과를 미리 보고 오류 위험을 줄이기를 권장합니다 ➝</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            <div class="info-item">
              <span class="info-label">이벤트 외형 나이</span>
              <span class="info-value">{{ selectedProfile.age || '-' }} 세</span>
            </div>
            <div class="info-item">
              <span class="info-label">이벤트 외형 성별</span>
            ...
            <div class="info-item">
              <span class="info-label">이벤트 외형 MBTI</span>
          ...
          <div class="modal-section">
            ...
            <p class="section-bio">{{ selectedProfile.bio || '소개가 없습니다' }}</p>  
          </div>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              <div class="dimension-card">
                ...
                <span class="dim-desc">이 이벤트의 완전한 행로 궤적에서</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">행로 패턴 프로파일</span>
                <span class="dim-desc">경험 총결과와 행위 스타일 선호</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">독특한 기억 인상</span>
                <span class="dim-desc">기반으로 현실 시드 형성의 기억</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">사회 관계 네트워크</span>
                <span class="dim-desc">개체 링크와 상호작용 그래프</span>
              </div>
            </div>
...
watch(currentStage, (newStage) => {
  if (newStage === '생성Agent페르소나' || newStage === 'generating_profiles') {
    ...
    phase.value = 2
    // 진입 설정 생성 단계, 시작 라운드 질문 설정
    if (!configTimer) {
      addLog('시작생성듀얼플랫폼시뮬레이션설정...')
      startConfigPolling()
    }
  } else if (newStage === '준비 시뮬레이션 스크립트' || newStage === 'copying_scripts') {
    phase.value = 2 // 여전히 설정 단계에 속함
  }
})

// 설정 중 계산 자동 생성의 라운드 수 (하드코딩 기본값 사용 안 함)
const autoGeneratedRounds = computed(() => {
  ...
  if (!totalHours || !minutesPerRound) {
    return null // 설정 데이터가 완전하지 않을 때 null 반환
  }
  const calculatedRounds = Math.floor((totalHours * 60) / minutesPerRound)
  // 최대 라운드 수가 40 미만(추천 값)인지 보장, 슬라이드 범위 예외 방지
...
// 에 따라 agent_id 가져오기 응의 username
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    addLog(`시뮬레이션 시작，사용자 정의라운드 수: ${customMaxRounds.value} 라운드`)
  } else {
    // 사용자가 자동 생성의 라운드 수를 선택, max_rounds 파라미터를 전달하지 않음  
    addLog(`시뮬레이션 시작，사용자동설정라운드 수: ${autoGeneratedRounds.value} 라운드`)
  }
...
const startPrepareSimulation = async () => {
  if (!props.simulationId) {
    addLog('오류：simulationId가 누락되었습니다')  
  ...
  phase.value = 1
  addLog(`시뮬레이션인스턴스이미생성: ${props.simulationId}`)
  addLog('진행 중: 준비시뮬레이션환경...')
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('감지 이미 완료의 준비 작업, 직접 사용')  
        await loadPreparedData()
        return
      }
      
      taskId.value = res.data.task_id
      addLog(`준비작업이 시작되었습니다`)
      addLog(`  └─ Task ID: ${res.data.task_id}`)
      
      // 즉시 예상 Agent 총 수 설정 (prepare 인터페이스에서 돌아가기 값 가져오기)  
      if (res.data.expected_entities_count) {
        expectedTotal.value = res.data.expected_entities_count
        addLog(`부터 Zep 그래프 읽기 ${res.data.expected_entities_count} 개체`)  
        if (res.data.entity_types && res.data.entity_types.length > 0) {
          addLog(`  └─ 개체 유형: ${res.data.entity_types.join(', ')}`)
        }
      }
      
      addLog('시작 라운드 진행률 준비...')  
      // 시작 라운드 진행률 문의  
      startPolling()
      // 시작실시간가져오기 Profiles
      startProfilesPolling()
    } else {
      addLog(`준비실패: ${res.error || '알 수 없음오류'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    addLog(`준비예외: ${err.message}`)
    emit('update-status', 'error')
  }
}
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.status === 'completed' || data.status === 'ready' || data.already_prepared) {
        ...
        addLog(`✗ 준비실패: ${data.error || '알 수 없음오류'}`)
        stopPolling()
        stopProfilesPolling()
      }
    }
  } catch (err) {
    console.warn('라운드 문의 상태 실패:', err)  
  }
}
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // API 돌아가기 효과 값일 때만 업데이트, 이미의 효과 값 덮어쓰기 방지  
      ...
      // Profile 생성 진행률 로그 출력 (수량 변화 시에만)  
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        if (currentCount === 1) {
          addLog(`시작생성Agent페르소나...`)
        }
        addLog(`→ Agent페르소나 ${currentCount}/${total}: ${profileName} (${latestProfile?.profession || '알 수 없음직업'})`)
        
        // 만약 모두 생성 완료  
        if (expectedTotal.value && currentCount >= expectedTotal.value) {
          addLog(`✓ 모두 ${currentCount} 개 Agent 페르소나 생성 완료`)  
        }
      }
    }
  } catch (err) {
    console.warn('가져오기 Profiles 실패:', err)
  }
}
    ...
    if (res.success && res.data) {
      ...
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        ...
        if (data.generation_stage === 'generating_profiles') {
          addLog('진행 중: 생성Agent페르소나설정...')
        } else if (data.generation_stage === 'generating_config') {
          addLog('진행 중: 호출LLM생성시뮬레이션설정파라미터...')
        }
      }
      ...
      if (data.config_generated && data.config) {
        ...
        // 상세 설정 요약 표시  
        if (data.summary) {
          addLog(`  ├─ Agent수량: ${data.summary.total_agents}개`)
          ...
          addLog(`  └─ 플랫폼설정: Twitter ${data.summary.has_twitter_config ? '✓' : '✗'}, Reddit ${data.summary.has_reddit_config ? '✓' : '✗'}`)
        }
        ...
        if (data.config.event_config?.narrative_direction) {
          ...
          addLog(`서사 방향: ${narrative.length > 50 ? narrative.substring(0, 50) + '...' : narrative}`)  
        }
        
        stopConfigPolling()
        phase.value = 4
        addLog('✓ 환경 설정완료，하여시뮬레이션 시작')
        emit('update-status', 'completed')
      }
    }
  } catch (err) {
    console.warn('가져오기 Config 실패:', err)
  }
}

const loadPreparedData = async () => {
  phase.value = 2
  addLog('진행 중: 로드이미설정데이터...')

  // 마지막으로 Profiles 가져오기  
  await fetchProfilesRealtime()
  addLog(`이미로드 ${profiles.value.length} 개Agent페르소나`)
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // 상세 설정 요약 표시  
        if (res.data.summary) {
          addLog(`  ├─ Agent수량: ${res.data.summary.total_agents}개`)
        ...
        // 설정이 아직 생성되지 않음, 시작 라운드 문의  
        addLog('설정 생성 중, 시작 라운드 문의 대기...')  
        startConfigPolling()
      }
    }
  } catch (err) {
    addLog(`로드설정실패: ${err.message}`)
    emit('update-status', 'error')
  }
}
