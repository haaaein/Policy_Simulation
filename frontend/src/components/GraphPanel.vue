    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'GraphRAG장단기 기억실시간 업데이트 중' : '실시간 업데이트 중...' }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">아직 소량 내용 처리 중, 잠시 후 수동 그래프 새로고침</span>
      ...
      <div v-else class="graph-state">
        ...
        <p class="empty-text">대기온톨로지 생성...</p>
      </div>
    </div>
...
const showEdgeLabels = ref(true) // 기본값 표시 엣지 레이블
const expandedSelfLoops = ref(new Set()) // 전개된 자기 루프 항목
const showSimulationFinishedHint = ref(false) // 시뮬레이션종료 후안내
const wasSimulating = ref(false) // 추적 전에 시뮬레이션 중
...
// 리스닝 isSimulating 변화, 감지 시 시뮬레이션 종료
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // 부터 시뮬레이션 중 변로 비시뮬레이션 상태, 표시 종료 안내
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// 자기 루프 항목 전개/접기 상태 전환
...
const closeDetailPanel = () => {
  ...
  expandedSelfLoops.value = new Set() // 초기화 전개 상태
}
...
const renderGraph = () => {
  ...
  // 처리 엣지 데이터, 계산 동일 노드 간의 엣지 수량과 인덱스
  const edgePairCount = {}
  const selfLoopEdges = {} // 노드별 그룹화된 자기 루프 엣지
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // 통계 각 노드 간의 엣지 수량, 수집 자기 루프 엣지
  tempEdges.forEach(e => {
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // 자기 루프 엣지 - 각 노드에 대해 하나의 합병 자기 루프만 추가
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // 저장된 자기 루프 엣지의 상세 정보
        }
      })
      return
    }
    ...
    // 엣지의 방향 여부와 표준화 방향 일치 여부 판단（원UUID < 목표UUID）
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // 곡률 계산：여러 엣지일 때 분산, 단일 엣지로 직선
    let curvature = 0
    if (totalCount > 1) {
      // 균일 분포 곡률, 보장 명확한 영역 분할
      // 곡률 범위에 따라 엣지 수량 증가, 엣지가 많을수록 곡률 범위가 커짐
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // 만약 엣지의 방향과 표준화 방향이 반대라면, 곡률 반전
      // 이렇게 보장된 엣지에서 동일한 기준계 하에 분포, 방향이 다름에 따른 오버랩 방지
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // 시뮬레이션 - 에 따라 엣지 수량 동적으로 조정 노드 간격
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // 에 따라 이 노드 간의 엣지 수량 동적으로 조정 거리
      // 기본 거리 150, 하나의 엣지가 추가될 때마다 40 증가
  ...
  const getLinkPath = (d) => {
    ...
    // 감지 자가 루프
    if (d.isSelfLoop) {
      // 자가 루프: 노드에서 출발하여 다시 돌아가는 원호 그리기
      const loopRadius = 30
      // 노드 오른쪽에서 출발하여 한 바퀴 돌아감
      const x1 = sx + 8  // 시작점 오프셋
      const y1 = sy - 4
      const x2 = sx + 8  // 끝점 오프셋
      const y2 = sy + 4
      // 원호를 사용하여 자가 루프 그리기 (sweep-flag=1 시계 방향)
    ...
    // 곡선 제어점 계산 - 에 따라 변의 수량과 거리 동적으로 조정
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // 연결선 방향에 수직인 오프셋, 에 따라 거리 비율 계산, 곡선이 뚜렷하게 보이도록 보장
    // 변이 많을수록, 오프셋이 거리의 비율을 더 많이 차지함
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // 기본 25%, 변이 하나 늘어날 때마다 5% 증가
  ...
  // 링크 레이블 배경 (흰색 배경으로 글자가 더 선명하게 보임)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // 위치만 기록, 시뮬레이션 재시작 안 함 (영역 클릭 및 드래그)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // 진짜 드래그 시작 감지 (이동 초과 임계값)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // 최초로 진짜 드래그 감지 시에만 시뮬레이션 재시작
          d._isDragging = true
  ...
  simulation.on('tick', () => {
    ...
    // 변 레이블 위치 업데이트 (회전 없음, 수평 표시가 더 선명함)
