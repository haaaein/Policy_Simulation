    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            LLM분석문서내용와시뮬레이션 요구사항，추출현실 시드，자동생성적합한온톨로지구조  
          </p>

          <!-- Loading / Progress -->
          <div v-if="currentPhase === 0 && ontologyProgress" class="progress-section">
            <div class="spinner-sm"></div>
            <span>{{ ontologyProgress.message || '진행 중: 분석문서...' }}</span>
          </div>
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">GraphRAG구축</span>
          </div>
          <div class="step-status">
        ...
        <div class="card-content">
          ...
          <p class="description">
            기반으로생성의온톨로지，문서자동 청크 분할 후 호출 Zep 구축지식그래프，추출개체와관계，시간순 기억과 커뮤니티 형성요약
          </p>
          ...
          <div class="stats-grid">
            <div class="stat-card">
              ...
              <span class="stat-label">개체 노드</span>
            </div>
            <div class="stat-card">
...
const handleEnterEnvSetup = async () => {
  if (!props.projectData?.project_id || !props.projectData?.graph_id) {
    console.error('프로젝트 또는 그래프 정보가 부족합니다')
    return
  }
  ...
  try {
    ...
    if (res.success && res.data?.simulation_id) {
      ...
      console.error('생성시뮬레이션실패:', res.error)
      alert('생성시뮬레이션실패: ' + (res.error || '알 수 없음오류'))
    }
  } catch (err) {
    console.error('생성시뮬레이션예외:', err)
    alert('생성시뮬레이션예외: ' + err.message)
  } finally {
