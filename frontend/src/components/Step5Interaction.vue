          ...
          <div class="action-bar-tabs">
            ...
            <div class="agent-dropdown" v-if="profiles.length > 0">
              ...
              >
                ...
                <span>{{ selectedAgent ? selectedAgent.username : '와월드내 임의 개체와대화' }}</span>
              ...
              <div v-if="showAgentDropdown" class="dropdown-menu">
                ...
                >
                  ...
                  <div class="agent-info">
                    ...
                    <span class="agent-role">{{ agent.profession || '알 수 없음직업' }}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                ...
                <div class="tools-card-subtitle">보고서 생성지능형체의빠른대화버전，호출 4종 전문도구，포함 PolicySim의 전체 메모리</div>
              </div>
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">InsightForge 심층귀속</div>
                    <div class="tool-desc">실제월드종자데이터와시뮬레이션환경상태, 결합Global/Local Memory메커니즘, 제공시공간의심층귀속분석</div>
                  </div>
                </div>
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">PanoramaSearch 전경추적</div>
                    <div class="tool-desc">기반으로도형구조의넓이탐색알고리즘，재구성이벤트전파경로，포착전체정보흐름의토폴로지구조</div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">QuickSearch 빠른검색</div>
                    <div class="tool-desc">기반으로GraphRAG의즉시조회인터페이스，최적화인덱스효율，용빠른추출구체의노드속성과불연속사실</div>
                  </div>
                </div>
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">InterviewSubAgent 가상인터뷰</div>
                    <div class="tool-desc">자주식인터뷰，병렬과시뮬레이션월드중개체진행다중라운드대화，수집비구조화의관점데이터와심리상태</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="profile-card-header">
              ...
              <div class="profile-card-info">
                ...
                <div class="profile-card-meta">
                  ...
                  <span class="profile-card-profession">{{ selectedAgent.profession || '알 수 없음직업' }}</span>
                </div>
              </div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? '와 Report Agent 대화，심층해보고서내용' : '와시뮬레이션개체대화，해그들의관점' }}
              </p>
            </div>
            <div 
        ...
        <div v-if="activeTab === 'survey'" class="survey-container">
          ...
          <div class="survey-setup">
            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">조사대상선택</span>
                <span class="selection-count">이미선택 {{ selectedAgents.size }} / {{ profiles.length }}</span>
              </div>
              <div class="agents-grid">
                ...
                >
                  ...
                  <div class="checkbox-info">
                    ...
                    <span class="checkbox-role">{{ agent.profession || '알 수 없음직업' }}</span>
                  </div>
              ...
              <div class="selection-actions">
                <button class="action-link" @click="selectAllAgents">전체선택</button>
                <span class="action-divider">|</span>
                <button class="action-link" @click="clearAgentSelection">비우기</button>
              </div>
            </div>

            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">설문문제</span>
              </div>
              <textarea 
                v-model="surveyQuestion"
                class="survey-input"
                placeholder="입력당신이묻고싶은선택된대상의문제..."
                rows="3"
              ></textarea>
            </div>
          ...
          <div v-if="surveyResults.length > 0" class="survey-results">
            <div class="results-header">
              <span class="results-title">조사결과</span>
              ...
              >
                <div class="result-header">
                  ...
                  <div class="result-info">
                    ...
                    <span class="result-role">{{ result.profession || '알 수 없음직업' }}</span>
                  </div>
                </div>
...
const chatHistoryCache = ref({}) // 캐시된대화기록: { 'report_agent': [], 'agent_0': [], 'agent_1': [], ... }
...
const renderMarkdown = (content) => {
  ...
  // <p><br> 블록요소의상황（여분의공백행발생）
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // 연속의 <br> 태그정리
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // 블록요소후즉시따르는단락시작태그전의 <br>
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // 비연속순서목록의번호수정：단일항목 <ol> 단락내용이격리될때，번호증가유지
...
const sendMessage = async () => {
  ...
  try {
    ...
    addLog(`전송실패: ${err.message}`)
    chatHistory.value.push({
      role: 'assistant',
      content: `죄송합니다，오류가발생했습니다: ${err.message}`,
      timestamp: new Date().toISOString()
    })
  } finally {
...
const sendToReportAgent = async (message) => {
  addLog(`Report Agent에전송: ${message.substring(0, 50)}...`)
  ...
  if (res.success && res.data) {
    ...
    addLog('Report Agent 이미 답변함')
  } else {
    throw new Error(res.error || '요청실패')
  }
}

const sendToAgent = async (message) => {
  if (!selectedAgent.value || selectedAgentIndex.value === null) {
    throw new Error('먼저 하나의 시뮬레이션 개체를 선택하세요')
  }
  
  addLog(`에게 ${selectedAgent.value.username} 전송: ${message.substring(0, 50)}...`)
  ...
  if (res.success && res.data) {
    // 올바른 데이터 경로: res.data.result.results 하나의 객체 사전
    // 형식: {"twitter_0": {...}, "reddit_0": {...}} 또는 단일 플랫폼 {"reddit_0": {...}}
    const resultData = res.data.result || res.data
    const resultsDict = resultData.results || resultData
    
    // 객체 사전을 배열로 변환, 우선적으로 reddit 플랫폼의 응답 가져오기
    let responseContent = null
    const agentId = selectedAgentIndex.value
    
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // 우선적으로 reddit 플랫폼 응답 사용, 그 다음 twitter
    ...
    if (responseContent) {
      ...
      addLog(`${selectedAgent.value.username} 이미 답변함`)
    } else {
...
const submitSurvey = async () => {
  ...
  addLog(`전송설문 ${selectedAgents.value.size} 개 객체...`)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`수 ${surveyResults.value.length} 개 응답`)
    } else {
      throw new Error(res.error || '요청실패')
    }
  } catch (err) {
...
const loadReportData = async () => {
  ...
  try {
    addLog(`로드보고서데이터: ${props.reportId}`)
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`로드 ${profiles.value.length} 개 시뮬레이션 개체`)
    }
  } catch (err) {
    addLog(`로드 시뮬레이션 개체 실패: ${err.message}`)
  }
}
...
/* 왼쪽 패널 - 보고서 스타일 (와 Step4Report.vue 완전 일치) */
.left-panel.report-style {
  width: 45%;
  min-width: 450px;
  background: #FFFFFF;
...
/* 수정 목록 번호 - 사용 CSS 카운터 여러 개 ol 연속 번호 */
.message-text {
