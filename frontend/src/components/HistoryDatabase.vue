<template>
  ...
  >
    <!-- 배경 장식：기술 그리드 라인（있을 때만 프로젝트 시 표시） -->
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        <!-- 카드 헤더：simulation_id 와 기능 사용 가능상태 -->
        ...
        <!-- 카드 제목(사용 시뮬레이션 요구사항의 앞 20자로 제목) -->
        <h3 class="card-title">{{ getSimulationTitle(project.simulation_requirement) }}</h3>

        <!-- 카드 설명（시뮬레이션 요구사항 완전 표시） -->
    ...
    <div v-if="loading" class="loading-state">
      ...
      <span class="loading-text">로드 중...</span>
    </div>
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="modal-section">
                ...
                <div class="modal-empty" v-else>연관 파일이 없습니다</div>
              </div>
            </div>
            ...
            <div class="modal-divider">
              ...
              <span class="divider-text">재현 재생</span>
              <span class="divider-line"></span>
            </div>
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3「시뮬레이션 시작」와 Step5「심층 상호작용」에서 실행 중 시작, 지원하지 않음 역사 재생</span>
            </div>
          </div>
        </div>
      </Transition>
...
const selectedProject = ref(null)  // 현재 선택의 프로젝트（용 팝업）
let observer = null
let isAnimating = false  // 애니메이션 잠금, 깜박임 방지
let expandDebounceTimer = null  // 디바운스 타이머
let pendingState = null  // 기록 대 실행의 목표 상태

// 카드 레이아웃 설정 - 조정으로 더 넓은 비율
...
const containerStyle = computed(() => {
  ...
  // 실제 필요한 높이 계산：행 수 * 카드 높이 + (행 수-1) * 간격 + 소량 바닥 간격
...
// 시뮬레이션 요구사항 생성 제목(앞 20자 가져오기)
const getSimulationTitle = (requirement) => {
  if (!requirement) return '이름 없음시뮬레이션'
  const title = requirement.slice(0, 20)
  return requirement.length > 20 ? title + '...' : title
}

// 형식화 simulation_id 표시(앞 6자리 가져오기)
...
// 형식화 라운드 수 표시(현재 라운드/총 라운드 수)
const formatRounds = (simulation) => {
  const current = simulation.current_round || 0
  const total = simulation.total_rounds || 0
  if (total === 0) return '시작되지 않음'
...
const loadHistory = async () => {
  try {
    ...
    console.error('로드 역사 프로젝트 실패:', error)
    projects.value = []
  } finally {
    loading.value = false
  }
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // 업데이트 대기 실행의 목표 상태(무관하게 애니메이션 중에도 최신의 목표 상태 기록)
        pendingState = shouldExpand
        
        // 이전의 방지 타이머 제거(새로운 스크롤 의도가 이전 것을 덮어씀)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // 만약 진행 중: 애니메이션 중, 상태만 기록, 애니메이션 종료 후 처리
        if (isAnimating) return
        
        // 만약 목표 상태와 현재 상태가 동일하면 처리할 필요 없음
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // 방지 지연 상태 전환 사용, 빠른 깜박임 방지
        // 펼칠 때 지연이 짧음(50ms), 수축할 때 지연이 길음(200ms)으로 안정성 증가
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // 대기 실행 상태가 여전히 실행해야 하는지 확인(이미 후속 스크롤이 덮어씀)
          ...
          // 애니메이션 완료 후 잠금 해제, 대기 처리 상태 변화 확인
          setTimeout(() => {
            isAnimating = false
            
            // 애니메이션 종료 후, 새로운 대기 실행 상태 확인
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // 잠시 지연 후 다시 실행, 너무 빠른 전환 방지
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    {
      // 여러 개의 임계값 사용, 감지를 더 부드럽게
      threshold: [0.4, 0.6, 0.8],
      // rootMargin 조정, 뷰포트 바닥을 위로 수축, 더 많이 스크롤해야 펼쳐짐
...
onMounted(async () => {
  // DOM 렌더링 완료 후 데이터 다시 로드 보장
  await nextTick()
  await loadHistory()
  
  // DOM 렌더링 후 초기화 관찰기 대기
  setTimeout(() => {
    initObserver()
  }, 100)
})

// 만약 사용 keep-alive, 컴포넌트 활성화 시 데이터 다시 로드
onActivated(() => {
  loadHistory()
})

onUnmounted(() => {
  // 정리 Intersection Observer
  if (observer) {
    observer.disconnect()
    observer = null
  }
...
/* CSS 배경 패턴 생성 고정 간격의 정사각형 그리드 사용 */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* 왼쪽 상단 모서리에서 시작 위치 지정, 높이 변화 시 바닥만 확장, 이미 그리드 위치에 영향 없음 */
...
.cards-container {
  ...
  /* min-height는 JS로 동적으로 계산, 카드 수량에 따라 자동 조정 */
}
...
.status-icon:nth-child(2).available { color: #F59E0B; } /* 환경 설정 - 주황색 */
...
.card-progress.not-started { color: #9CA3AF; }  /* 시작 안 함 - 회색 */
