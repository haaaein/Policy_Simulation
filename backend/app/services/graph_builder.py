...
class GraphBuilderService:
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        ...
        # Zep SDK에서 요구하는 사용법이며, 동적 클래스 생성에서 발생하는 경고로 안전하게 무시 가능
        ...
        for entity_def in ontology.get("entity_types", []):
            ...
            # 속성 딕셔너리와 타입 어노테이션 생성 (Pydantic v2 필요)
            ...
            for attr_def in entity_def.get("attributes", []):
                ...
                # Zep API는 Field의 description이 필수
        ...
        for edge_def in ontology.get("edge_types", []):
            ...
            for attr_def in edge_def.get("attributes", []):
                ...
                # Zep API는 Field의 description이 필수
                attrs[attr_name] = Field(description=attr_desc, default=None)
                annotations[attr_name] = Optional[str]  # 엣지 속성은 str 타입
    ...
    def add_text_batches(
        ...
        """배치로 텍스트를 그래프에 추가, 모든 episode의 uuid 리스트 반환"""
        episode_uuids = []
        total_chunks = len(chunks)
        ...
        for i in range(0, total_chunks, batch_size):
            ...
            if progress_callback:
                ...
                progress_callback(
                    f"제 {batch_num}/{total_batches} 배치 데이터 전송 중 ({len(batch_chunks)}개 청크)...",
                    progress
                )
    ...
    def _wait_for_episodes(
        ...
        while pending_episodes:
            if time.time() - start_time > timeout:
                if progress_callback:
                    progress_callback(
                        f"일부 텍스트 청크 타임아웃, {completed_count}/{total_episodes} 완료",
            ...
            if pending_episodes:
                time.sleep(5)  # 5초마다 확인 (API 부하 감소)
