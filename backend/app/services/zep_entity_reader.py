...
Zep 그래프에서 노드를 읽고, 사전 정의된 엔티티 타입에 부합하는 노드를 선별
"""
...
class ZepEntityReader:
    ...
    2. 사전 정의된 엔티티 타입에 부합하는 노드 선별 (Labels가 Entity만이 아닌 노드)
    ...
    def get_node_edges(self, node_uuid: str) -> List[Dict[str, Any]]:
        """
        지정 노드의 모든 관련 엣지 조회 (재시도 메커니즘 포함)
    ...
    def filter_defined_entities(
        ...
        """
        사전 정의된 엔티티 타입에 부합하는 노드 선별

        선별 로직:
        - 노드의 Labels가 "Entity" 하나뿐이면, 사전 정의된 타입에 부합하지 않으므로 건너뜀
        - 노드의 Labels에 "Entity"와 "Node" 외의 라벨이 포함되면, 사전 정의된 타입에 부합하므로 유지

        Args:
            graph_id: 그래프 ID
            defined_entity_types: 사전 정의된 엔티티 타입 리스트 (선택, 제공 시 해당 타입만 유지)
            enrich_with_edges: 각 엔티티의 관련 엣지 정보 조회 여부

        Returns:
            FilteredEntities: 필터링된 엔티티 집합
        """
        logger.info(f"그래프 {graph_id}의 엔티티 선별 시작...")
        ...
        for node in all_nodes:
            ...
            # 선별 로직: Labels에 "Entity"와 "Node" 외의 라벨이 포함되어야 함
        ...
        logger.info(f"선별 완료: 총 노드 {total_count}, 조건 부합 {len(filtered_entities)}, "
    ...
    def get_entity_with_context(
        ...
        """
        단일 엔티티 및 전체 컨텍스트 조회 (엣지와 연관 노드, 재시도 메커니즘 포함)
